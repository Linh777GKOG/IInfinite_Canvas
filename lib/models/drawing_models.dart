...
enum ActiveTool { brush, eraser, hand, image, text, shape, lasso }
...
class CanvasText {
  ...
  factory CanvasText.fromJson(Map<String, dynamic> json) {
    ...
    final weightIndex =
        (json['fontWeight'] as num?)?.toInt() ?? FontWeight.w600.index;
    ...
    return CanvasText(
      ...
      fontWeight:
          FontWeight.values[weightIndex.clamp(0, FontWeight.values.length - 1)],
...
class ImportedImagePersisted {
  ...
  Map<String, dynamic> toJson({required bool webInlineImages}) {
    return {
      ...
      'scale': scale,
      'rotation': rotation,
      if (webInlineImages) 'bytes': bytes == null ? null : base64Encode(bytes!),
...
class DrawingDocument {
  ...
  Map<String, dynamic> toJson({required bool webInlineImages}) {
    return {
      ...
      'images': images
          .map((i) => i.toJson(webInlineImages: webInlineImages))
          .toList(),
  ...
  factory DrawingDocument.fromJson(
    ...
    final imagesList = (json['images'] as List<dynamic>? ?? const [])
        .map(
          (e) => ImportedImagePersisted.fromJson(
