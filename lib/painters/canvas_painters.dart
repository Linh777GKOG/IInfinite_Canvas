...
class DrawPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final t in texts) {
      ...
      textPainter.paint(
        canvas,
        t.backgroundColor == null ? Offset.zero : Offset(t.padding, t.padding),
      );
      canvas.restore();
    }
  }
      ...
      for (double x = 0; x <= size.width; x += gridSize) {
        for (double y = 0; y <= size.height; y += gridSize) {
          canvas.drawCircle(
            Offset(x, y),
            1.2,
            majorPaint..style = PaintingStyle.fill,
          );
        }
      }
    }
...
class LassoPainter extends CustomPainter {
  ...
  @override
  void paint(Canvas canvas, Size size) {
    final strokeW = (2.0 / viewportScale).clamp(1.0, 3.0);
    final paint = Paint()
